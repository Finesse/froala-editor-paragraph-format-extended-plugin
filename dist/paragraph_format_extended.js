/**
 * Froala Editor Paragraph Format Extended plugin v0.1.1 (https://github.com/FinesseRus/froala-editor-paragraph-format-extended-plugin)
  ...
  */
   ...
   */
		html: function html() {
			...
			return '<ul class="fr-dropdown-list">' + this.opts.paragraphFormatExtended.map(function (format) {
				var title = _this.language.translate(format.title);
				var tag = format.tag || _this.html.defaultTag();
				var formatId = getFormatId(format);

				return '<li>' + '<' + tag + (format['class'] ? ' class="' + format['class'] + '"' : '') + ' style="padding: 0 !important; margin: 0 !important;">' + '<a class="fr-command" data-cmd="paragraphFormatExtended" data-param1="' + formatId + '" title="' + title + '">' + title + '</a>' + '</' + tag + '>' + '</li>';
			}).join("\n") + '</ul>';
		},
